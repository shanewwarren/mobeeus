...
//    - creates a publisher for each queue name defined in the options.
//    - creates a simplified interface for scheduling jobs.
...
exports.register = function (server, options, next) {
    ...
    Joi.assert(options, PluginSchema, 'Bad plugin options passed to Mobeeus.');
    ...
    if (!rootState.setup) {
        ...
        server.decorate('server', 'dispatcher', internals.collections((ctx) => ctx, 'realm'));
        server.decorate('request', 'dispatcher', internals.collections((ctx) => ctx.server, 'route.realm'));
